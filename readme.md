# KeyNexus
> 이 프로젝트는 SKALA MSA 교육 실습 목적으로 진행되었습니다.
...
| 팀원 | 주요 기여 영역 |
| --- | --- |
| 이윤서 (`Hidy`) | (PM) 프로젝트 접근 권한 회수, 서비스 연동, 문서화 |
| 목진훈 (`komlab`) | (BackEnd) 프로젝트·자산 및 접근 신청 API, 인증 호환, 통합·데모 환경 |
| 박현준 `hxxnjoon` | (FronEnd) Vue 프론트엔드 화면·라우팅, 인증 UI, 로컬 실행 환경 |
| 이휘호`hwiho` | (BackEnd)위험도 분석, Credential 감사 이력, 서비스 간 인증·승인 연동 |
